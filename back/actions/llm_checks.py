...
"""
    ...
    # --- 4) Define the JSON schema for an array of DimensionStatus objects ---
    # Define the schema for a single item in the array
    ...
    try:
        for chunk in client.models.generate_content_stream(
            ...
            # Append each streamed chunk's text to build the full response
            response_str += chunk.text

        log.info(f"Collected streamed response from Gemini (length: {len(response_str)}).")
    except Exception as e:
        log.error(f"Error calling Gemini API (streaming): {e}\n{traceback.format_exc()}")
        return results
